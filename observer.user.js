    ...
    mutations.forEach(function (m) {
        ...
        if (m.type == 'attributes') {
            if (m.attributeName == 'style') {
                ...
                add_diff(Changes, '@' + m.attributeName, m.oldValue, m.target.attributes[m.attributeName].value, m.target);
            }
        } else if (m.type == 'childList') {
            add_diff(Changes, NODE_CHANGES, m.removedNodes.length, m.addedNodes.length);
        }
    });
    console.log(Changes);
    GM_xmlhttpRequest({
...
function add_diff(Changes, attrName, oldAttr, newAttr, target) {
    ...
    if (attrName == NODE_CHANGES) {
        ...
        Changes.deltas.push([attrName,
        nodeNum,
        oldAttr,
        newAttr]);
        console.log("Added delta, Changes: " + Changes);
    }
}
